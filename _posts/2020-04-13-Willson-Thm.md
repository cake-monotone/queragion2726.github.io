...
$p(x)$의 항 개수, 그리고 $N!$의 항 개수를 보면 각각 $\sqrt{N} = m$개로 잘 나눠진
것을 알 수 있습니다.
...
$$ p(x)\ mod \ (x-a) \\ p(x)\ mod \ (x-b) \\ p(x)\ mod \ (x-c) \\ p(x)\ mod\
(x-d) $$
...
$$ p(x)\ mod\ q(x) \\ = \Big( p(x)\ mod\ q(x)r(x) \Big)\ mod\ q(x) $$
...
중요한건 $p(x)\ mod\ ?$ 를 직접 계산하지 않아야 합니다. 각각의 노드에서 계산한
나머지 값을 자식에게 넘겨주며, 과정을 반복합니다.

$$ r_{00}(x) = p(x)\ mod\ (x-a)(x-b)(x-c)(x-d) $$

예시의 루트 노드를 예로 들어봅시다. $r_{00}(x)$를 계산하고 자식노드에게는 $p(x)$
대신 $r_{00}(x)$를 넘겨 줍니다.

$$ r_{10}(x)= r_{00}(x)\ mod\ (x-a)(x-b) \\ r_{11}(x) = r_{00}(x)\ mod\
(x-c)(x-d) $$

자식노드는 $r_{00}(x)$를 받아 똑같은 과정을 반복합니다. 리프노드까지 도달하면
계산이 완료됩니다.
