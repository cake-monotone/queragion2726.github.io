...
이 `bakery`를 사용해서 쿼리를 빌드할 수가 있는데, 이 과정에서 캐싱이 됩니다. 다음부터는 같은 쿼리를 빌드하지 않고, 바로 DB에 보낼 수 있습니다. 

캐싱하는 과정을 빵을 굽는것에 비유를 한건데, 적절한지는 잘 모르겠지만 귀엽다는 생각은 드네요. 그럼 따끈따끈한 쿼리를 구워봅시다.
...
다만 람다를 사용하고, 캐싱을 한다는 특성 상 몇몇 문제가 발생합니다. 이 부분도 포스트에 있으니 참고해주세요.
